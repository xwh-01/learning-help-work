...
项目支持两种运行模式：**Docker Compose** 和 **Local**。两种模式**不能混用**。

| 组件 | Docker 模式 | Local 模式 |
|------|-------------|------------|
...
# 编辑 backend/.env 填入真实 LLM API Key
...
需要本机已安装并启动 MySQL 8.x、Redis 7.x。
...
Celery worker 被 Ctrl+C 或崩溃可能导致 async_tasks 永久卡在 `running`，
learning_sessions 永久卡在 `generating`。
...
# 修复卡住超过 5 分钟的任务
python scripts/repair_stale_tasks.py --older-than-minutes 5
```

恢复逻辑：
- 已生成的产物（knowledge_points / examples / levels）不会删除也不会重复生成
- 产物足够的 session 修正为 `ready`
- 产物不足的 task 标记为 `failed` 并写入 error_message

## 当前已实现

- learning session 创建与异步生成
- async task 状态追踪
- official material fetch（官方文档抓取 + LLM 总结）
- LLM comparison（技术边界对比）
- knowledge points（知识点拆解）
- examples（体感样例生成）
- levels（observe / hands_on / summary 三类关卡）
- answer feedback（答案反馈）
- practice task（实战 Boss 题）
- learning card（技术卡片）

## 当前待完善

- LLM JSON repair（真实 LLM 输出不稳定时的容错修复）
- async task recovery（Celery 崩溃后的 stale task 恢复）
- resume / breakpoint continuation（断点续跑，已生成部分不重复）
- 子任务级进度（例如 level 12/18 细粒度进度）
